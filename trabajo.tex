...
\section{A* and weighted A* algorithms}
a
\subsection{A* algorithm}
a
\subsection{Weighted A* algorithm}
a
\section{A* and weighted A* applied to the 8-Puzzle problem}
a
\subsection{Heuristics}
The heuristics we will be using are the following:
\begin{enumerate}
\item h0: Null heuristic. It is not informed.
\item h1: Weighted Manhattan distance. The Manhattan distance between the goal and the current locations is the sum of the absolute differences of their Cartesian coordinates.
\item h2: Weighted misplaced tiles. The number of tiles that are not on the goal location.
\end{enumerate}
"Weighted" heuristics refers to the version of the heuristic for the problem in which the cost of the rules is \(2^i\)
